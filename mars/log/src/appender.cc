...
void XloggerAppender::Open(const XLogConfig& _config) {
    ...
    if (OpenMmapFile(mmap_file_path, kBufferBlockLength, mmap_file_))  {
	    if (_config.compress_mode_ == kZstd){
		    log_buff_ = new LogZstdBuffer(mmap_file_.data(), kBufferBlockLength, _config.compress, _config.pub_key_.c_str(), _config.compress_level_);
	    }else {
		    log_buff_ = new LogZlibBuffer(mmap_file_.data(), kBufferBlockLength, _config.compress, _config.pub_key_.c_str());
	    }
        use_mmap = true;
    } else {
        char* buffer = new char[kBufferBlockLength];
	    if (_config.compress_mode_ == kZstd){
		    log_buff_ = new LogZstdBuffer(buffer, kBufferBlockLength,  _config.compress, _config.pub_key_.c_str(), _config.compress_level_);
	    } else {
		    log_buff_ = new LogZlibBuffer(buffer, kBufferBlockLength,  _config.compress, _config.pub_key_.c_str());
	    }
        use_mmap = false;
    }
