    ...
    if (classNameStr != NULL) {
        ...
        if (classNameChars != NULL) {
            ...
            jmethodID throwableGetMessageMethod =
                env->GetMethodID(exceptionClazz, "getMessage", "()Ljava/lang/String;");
...
static void printStackTrace(JNIEnv* env, jthrowable exception, char* buf, size_t bufLen) {
    ...
    if (stringWriterClazz != NULL) {
        jmethodID stringWriterCtor = env->GetMethodID(stringWriterClazz, "<init>", "()V");
        jmethodID stringWriterToStringMethod = env->GetMethodID(stringWriterClazz, "toString", "()Ljava/lang/String;");
        jclass printWriterClazz = env->FindClass("java/io/PrintWriter");
        if (printWriterClazz != NULL) {
            jmethodID printWriterCtor = env->GetMethodID(printWriterClazz, "<init>", "(Ljava/io/Writer;)V");
            jobject stringWriterObj = env->NewObject(stringWriterClazz, stringWriterCtor);
            if (stringWriterObj != NULL) {
                jobject printWriterObj = env->NewObject(printWriterClazz, printWriterCtor, stringWriterObj);
                if (printWriterObj != NULL) {
                    jclass exceptionClazz = env->GetObjectClass(exception);  // can't fail
                    jmethodID printStackTraceMethod =
                        env->GetMethodID(exceptionClazz, "printStackTrace", "(Ljava/io/PrintWriter;)V");
                    env->CallVoidMethod(exception, printStackTraceMethod, printWriterObj);
                    if (!env->ExceptionCheck()) {
                        jstring messageStr =
                            (jstring)env->CallObjectMethod(stringWriterObj, stringWriterToStringMethod);
                        if (messageStr != NULL) {
...
//
jvalue __JNU_CallMethodByName(JNIEnv* env, jobject obj, const char* name, const char* descriptor, va_list args) {
    ASSERT(env != NULL);
    ASSERT(obj != NULL);
    ASSERT(name != NULL);
 ...
 * @return
 */
jvalue JNU_CallMethodByName(JNIEnv* _env, jobject obj, const char* _name, const char* _descriptor, ...) {
    ASSERT(_env != NULL);
    ASSERT(obj != NULL);
    ASSERT(_name != NULL);
...
jvalue __JNU_CallStaticMethodByName(JNIEnv* _env,
                                    jclass _clazz,
                                    const char* _name,
...
jvalue JNU_CallStaticMethodByName(JNIEnv* _env, jclass _clazz, const char* _name, const char* _descriptor, ...) {
    ASSERT(_env != NULL);
    ASSERT(_clazz != NULL);
    ASSERT(_name != NULL);
    ASSERT(_descriptor != NULL);
...
jvalue JNU_CallStaticMethodByMethodInfo(JNIEnv* _env, JniMethodInfo methodInfo, ...) {
    ...
    jvalue result =
        __JNU_CallStaticMethodByName(_env, _clazz, methodInfo.methodname.c_str(), methodInfo.methodsig.c_str(), args);
    va_end(args);
