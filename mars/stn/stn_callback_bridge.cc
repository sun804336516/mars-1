...
boost::signals2::signal<void(ErrCmdType _err_type, int _err_code, const std::string& _ip, uint16_t _port)>
    SignalOnLongLinkNetworkError;
boost::signals2::signal<
    void(ErrCmdType _err_type, int _err_code, const std::string& _ip, const std::string& _host, uint16_t _port)>
...
bool StnCallbackBridge::Req2Buf(uint32_t _taskid,
    ...
    // #if !defined(ANDROID) || defined(CPP_CALL_BACK)
    xassert2(sg_callback != NULL);
    return sg_callback->Req2Buf(_taskid,
                                _user_context,
                                _user_id,
                                outbuffer,
                                extend,
...
int StnCallbackBridge::OnTaskEnd(uint32_t _taskid,
    ...
    cgiprofile.start_tls_handshake_time =
        _profile.tls_handshake_successful_time == 0 ? 0 : _profile.start_tls_handshake_time;
    ...
    cgiprofile.send_packet_finished_time = _profile.start_send_packet_time + _profile.send_request_cost;
    ...
    cgiprofile.start_encode_packet_time = _profile.start_encode_packet_time;
    cgiprofile.encode_packet_finished_time = _profile.encode_packet_finished_time;
    cgiprofile.start_decode_packet_time = _profile.start_decode_packet_time;
    cgiprofile.decode_packet_finished_time = _profile.decode_packet_finished_time;
    ...
    cgiprofile.nettype = _profile.net_type;
...
//网络层收到push消息回调
void OnPush(const std::string& _channel_id, uint32_t _cmdid, uint32_t _taskid, const AutoBuffer& _body, const
AutoBuffer& _extend) { xassert2(sg_callback_bridge != NULL); sg_callback_bridge->OnPush(_channel_id, _cmdid, _taskid,
_body, _extend);
};

//底层获取task要发送的数据
bool Req2Buf(uint32_t taskid,  void* const user_context, const std::string& _user_id, AutoBuffer& outbuffer, AutoBuffer&
extend, int& error_code, const int channel_select, const std::string& host) { xassert2(sg_callback_bridge != NULL);
    return sg_callback_bridge->Req2Buf(taskid, user_context, _user_id, outbuffer, extend, error_code, channel_select,
host);
};
//底层回包返回给上层解析
int Buf2Resp(uint32_t taskid, void* const user_context, const std::string& _user_id, const AutoBuffer& inbuffer, const
AutoBuffer& extend, int& error_code, const int channel_select) { xassert2(sg_callback_bridge != NULL); return
sg_callback_bridge->Buf2Resp(taskid, user_context, _user_id, inbuffer, extend, error_code, channel_select);
};
//任务执行结束
int  OnTaskEnd(uint32_t taskid, void* const user_context, const std::string& _user_id, int error_type, int error_code,
const ConnectProfile& _profile) { xassert2(sg_callback_bridge != NULL); return sg_callback_bridge->OnTaskEnd(taskid,
user_context, _user_id, error_type, error_code, _profile);
};
...
void OnShortLinkNetworkError(ErrCmdType _err_type, int _err_code, const std::string& _ip, const std::string& _host,
uint16_t _port) { xassert2(sg_callback_bridge != NULL); SignalOnShortLinkNetworkError(_err_type, _err_code, _ip, _host,
_port); sg_callback_bridge->OnShortLinkNetworkError(_err_type, _err_code, _ip, _host, _port);
};
//长连信令校验 ECHECK_NOW = 0, ECHECK_NEVER = 1, ECHECK_NEXT = 2
int  GetLonglinkIdentifyCheckBuffer(const std::string& _channel_id, AutoBuffer& identify_buffer, AutoBuffer&
buffer_hash, int32_t& cmdid) { xassert2(sg_callback_bridge != NULL); return
sg_callback_bridge->GetLonglinkIdentifyCheckBuffer(_channel_id, identify_buffer, buffer_hash, cmdid);
};
//长连信令校验回包
bool OnLonglinkIdentifyResponse(const std::string& _channel_id, const AutoBuffer& response_buffer, const AutoBuffer&
identify_buffer_hash) { xassert2(sg_callback_bridge != NULL); return
sg_callback_bridge->OnLonglinkIdentifyResponse(_channel_id, response_buffer, identify_buffer_hash);
};
